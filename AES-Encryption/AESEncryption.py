...
def randomNumMatrixGen(rows:int,columns:into,upperNum: int, lowerNum: int)->list:
    mat = []
    for row in range(rows):
        mat.append([])
        for column in range(columns):
...
def subBytes(state:list) -> list:
    for column in range(len(state)):
        for row in range(len(state[0])):
            state[column][row] = subBoxTrans(state[column][row])
    return state
...
def encrypt(text:str,encryptKey:str,rounds:int):
    ...
    text = createMatrixFromText(splitText(text))
    for j in range(len(text)):
...
encrypt("Two One Nine Two we are getting high tonight wow","Thats my Kung Fu",10)